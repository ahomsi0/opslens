...
export function ConnectNeonDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await createConnection({ provider: "neon", name, token: token.trim() });
      reset();
      onOpenChange(false);
      router.refresh();
    } catch (err) {
  ...
  return (
    ...
    >
      <DialogContent className="max-w-md">
        <div>
          <h2 className="text-base font-semibold tracking-tight">Connect Neon</h2>
          <p className="mt-2 text-sm text-[var(--color-fg-muted)]">
            Paste a Neon API key. We&apos;ll pull your Postgres projects and
            their regions. Neon doesn&apos;t expose a deployment history (it&apos;s a managed
            database), so the deployments tab will be empty for these.
          </p>
        </div>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1.5 text-[11px] text-[var(--color-fg-subtle)]">
              Stored encrypted (AES-256-GCM). Read-only access is enough.
            </p>
          </div>
